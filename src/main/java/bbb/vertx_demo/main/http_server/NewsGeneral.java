      ...
      redisApi.get(GENERAL_NEWS_CACHED_CONTENT)
        ...
        .onSuccess(redisResponse -> {
            if (nonNull(redisResponse)) {
              ...
              var query = "SELECT category, datetime, headline, image, related, source, summary, url FROM finnhub.news_general_view WHERE datetime >= NOW() - INTERVAL '3 days' ORDER BY datetime DESC";
              pgConnection
                .query(query)
                .execute()
                .onFailure(throwable -> {
                ...
                .onSuccess(rowSet -> {
                    ...
                    renderingContext.put("tableSubheader", "A broad look at the latest financial headlines. This feed highlights economic trends and policy changes across sectors.");
