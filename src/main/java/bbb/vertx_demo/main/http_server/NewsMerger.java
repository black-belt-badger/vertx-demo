...
import static bbb.vertx_demo.main.NewsCategory.MERGER;
...
public enum NewsMerger {
  ...
  private static final String MERGER_NEWS_CACHED_CONTENT = "/merger-news";
  ...
  static Handler<RoutingContext> mergerNews
    ...
    return context -> {
      ...
      redisApi.get(MERGER_NEWS_CACHED_CONTENT)
            ...
            if (nonNull(redisResponse)) {
              ...
              pgConnection
                ...
                .onSuccess(rowSet -> {
                    ...
                    redisApi
                      .hget(EPOCH_MILLIS, MERGER.value)
                          ...
                          engine
                            ...
                            .onSuccess(buffer -> {
                                ...
                                var request = cmd(SETEX)
                                  .arg(MERGER_NEWS_CACHED_CONTENT)
                                  .arg(maxAge)
                                  .arg(buffer);
